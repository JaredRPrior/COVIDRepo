...
class City:
            ...
            for node_index in self.network_keys: # for every node
                if self.network.nodes[node_index]['state'] == INFECTED_STATE: #If that node is infected
                    ...
                    for neighbor in list(self.network.neighbors(node_index)): #Loop through all the neighbors of that node
                        threshold = self.beta
                        if SD == True:
                            threshold=self.beta*b
                        if(random.random() <= self.threshold and self.network.nodes[neighbor]['state'] == SUSCEPTIBLE_STATE): # If some random number is greater than beta and the person is not immune then we will infect the neighbor
    ...
    def introduce_infected_node(self):
        ...
        self.network.nodes[infect_index]['duration'] = self.sigma + random.randint(-1, 8)
        self.number_exposed += 1
